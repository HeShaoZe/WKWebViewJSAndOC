//
//  SCDCredPointsHeaderPropressView.m
//  SmartCityDemo2-DEV
//
//  Created by 何少泽(总行零售网络金融) on 2018/12/27.
//  Copyright © 2018年 平安银行-智慧城市项目组. All rights reserved.
//

#import "SCDCredPointsHeaderPropressView.h"
...
@interface SCDCredPointsHeaderPropressView()
...
@property (nonatomic,strong)UILabel *descriptionLabel;  //  进度文字
...
@implementation SCDCredPointsHeaderPropressView
...
- (UILabel *)descriptionLabel
{
    if (!_descriptionLabel)
    {
        ...
        CGFloat descriptionLabelH = CGRectGetHeight(self.progressView.frame);
        CGFloat descriptionLabelY = _centerY - height / 2 + descriptionLabelH;
        _descriptionLabel.frame = CGRectMake((self.frame.size.width - width) / 2, descriptionLabelY, width, height);
        _descriptionLabel.font = [UIFont systemFontOfSize:20];
        _descriptionLabel.backgroundColor = [UIColor clearColor];
        _descriptionLabel.textAlignment = NSTextAlignmentCenter;
        _descriptionLabel.textColor = [UIColor whiteColor];
        _descriptionLabel.text = @"信用极好";
...
- (CAShapeLayer *)drawBottomLayer
{
    ...
    _bottomShapeLayer.lineJoin = kCALineJoinRound;
...
- (CAShapeLayer *)drawUpperLayer
{
    ...
    _upperShapeLayer.strokeColor     = [UIColor whiteColor].CGColor;
...
- (CAGradientLayer *)drawGradientLayer
{
    ...
    NSMutableArray *colors = [NSMutableArray arrayWithObjects:(id)[UIColor whiteColor].CGColor,(id)[UIColor whiteColor].CGColor,nil];//[NSMutableArray arrayWithObjects:(id)[UIColor greenColor].CGColor,(id)[UIColor whiteColor].CGColor,(id)[UIColor purpleColor].CGColor,(id)[UIColor redColor].CGColor, nil];
...
- (void)setCardPoint:(NSInteger)cardPoint
{
    _cardPoint = cardPoint;
    self.percent = _cardPoint/1000.0;
    
    self.progressView.text = [NSString stringWithFormat:@"%ld",(long)_cardPoint];
    ...
    else if ((550 <= _cardPoint) && (_cardPoint < 600))
    {
        detailStr = @"中等";
    }
    else if ((600 <= _cardPoint) && (_cardPoint < 650))
    {
        detailStr = @"良好";
    }
    else if ((650 <= _cardPoint) && (_cardPoint < 700))
    {
        detailStr = @"优秀";
    }
    else if ((700 <= _cardPoint) && (_cardPoint < 950))
    {
        detailStr = @"极好";
    }
    self.descriptionLabel.text = [NSString stringWithFormat:@"信用%@",detailStr];
}
